...
object H2TaskManager {
  ...
  def startDatabase(paths: Seq[String]): Unit = {
    process match {
      case None =>
...
object AkkaoogleBuild extends Build with ConfigureScalaBuild {
  ...
  lazy val root: Project = scalaMiniProject("com.tfr", "Akkaoogle", "1.0")
      .settings(stopH2Task, stopH2Task)
      .settings(
        organization := "theFeralRooster",
        scalaVersion := "2.10.0",
        scalacOptions ++= Seq(
          "-unchecked",
          "-deprecation"
...
trait ConfigureScalaBuild {
  ...
  lazy val typesafe: MavenRepository = "Typesafe Repository" at "http://repo.typesafe.com/typesafe/releases/"

  lazy val typesafeSnapshot: MavenRepository = "Typesafe Snapshots Repository" at "http://repo.typesafe.com/typesafe/snapshots/"

  val netty = Some("play.core.server.NettyServer")

  def scalaMiniProject(org: String, name: String, buildVersion: String, baseFile: java.io.File = file(".")): Project = Project(id = name, base = baseFile, settings = Project.defaultSettings).settings(
